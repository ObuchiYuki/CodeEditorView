...
// MARK: - C++
// ===========================================================================
private let cppReservedKeywords: [String] = [
    // C++ keywords (including C subset)
...
private let cppReservedOperators: [String] = [
    // Arithmetic + assignment
    "+", "-", "*", "/", "%", "+=", "-=", "*=", "/=", "%=",
    // Increment / decrement
    "++", "--",
...
// We can reuse the C numeric/string/char definition with slight modifications for C++.
private let cppNumberRegex = cNumberRegex
private let cppStringRegex = cStringRegex
private let cppCharRegex = cCharRegex
private let cppIdentifierRegex = cIdentifierRegex

extension LanguageConfiguration {
    /// Language configuration for C++
    public static func cpp(_ languageService: LanguageService? = nil) -> LanguageConfiguration {
        return LanguageConfiguration(
            name: "C++",
            supportsSquareBrackets: true,
            supportsCurlyBrackets: true,
            indentationSensitiveScoping: false,
            stringRegex: cppStringRegex,
            characterRegex: cppCharRegex,
            numberRegex: cppNumberRegex,
            singleLineComment: "//",
            nestedComment: (open: "/*", close: "*/"),  // Standard block comments
            identifierRegex: cppIdentifierRegex,
            operatorRegex: nil,
            reservedIdentifiers: cppReservedKeywords,
            reservedOperators: cppReservedOperators,
            languageService: languageService
        )
    }
}
